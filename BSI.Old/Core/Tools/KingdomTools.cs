...
namespace BSI.Core.Tools
{
    class KingdomTools
    {
        ...
        public static Kingdom CreateKingdom(Hero leader, TextObject name, TextObject informalName, Banner banner = null, bool showNotification = false)
        {
            ...
            Color MainColor = new ColorHelper().Random;
            Color AltColor = new ColorHelper().Random;
            kingdom.InitializeKingdom(name, informalName, leader.Culture, banner ?? Banner.CreateRandomClanBanner(leader.StringId.GetDeterministicHashCode()), MainColor.ToUnsignedInteger(), ColorHelper.OpposingColor(MainColor).ToUnsignedInteger(), leader.Clan.InitialPosition);
            ...
            AccessTools.Property(typeof(Kingdom), "AlternativeColor2").SetValue(kingdom, ColorHelper.OpposingColor(MainColor).ToUnsignedInteger());

            GameManager.EventManager.AddFaction(kingdom);

            typeof(CampaignEventDispatcher).GetMethod("OnKingdomCreated", BindingFlags.NonPublic | BindingFlags.Instance).Invoke(CampaignEventDispatcher.Instance, new object[] { kingdom });
